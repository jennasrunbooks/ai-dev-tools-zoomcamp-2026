...
def _database_url() -> str:
    ...
    # SQLAlchemy needs the driver named explicitly to pick psycopg (v3) over
    # the legacy psycopg2 driver it otherwise defaults "postgresql://" to.
...
def immediate_transaction() -> Generator[Session, None, None]:
    ...
    reservation serializes claims, recovery, and terminal submissions across
    API processes, giving each task one active lease. PostgreSQL uses an
    ordinary transaction instead and relies on the row locks in ``with_lock``
    below (``FOR UPDATE`` / ``FOR UPDATE SKIP LOCKED``), applied per query in
    :mod:`storage` and :func:`recover_expired_in_session`.
    """
...
def with_lock(query, *, skip_locked: bool = False):
    """Add a row lock for backends that support one inside :func:`immediate_transaction`.

    A no-op on SQLite, which already serializes writers via ``BEGIN
    IMMEDIATE``. On PostgreSQL this is what gives a contended row exactly one
    writer instead of relying on a table/database-wide lock.
    """
...
def recover_expired_in_session(db: Session, now: datetime) -> int:
    ...
    for attempt in expired:
        task = db.scalar(with_lock(select(Task).where(Task.id == attempt.task_id)))
