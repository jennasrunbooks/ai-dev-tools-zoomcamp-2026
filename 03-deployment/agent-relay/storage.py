...
def create_task(sender_id: str, recipient_id: str, input_text: str, idempotency_key: str | None) -> dict[str, str]:
    ...
    with immediate_transaction() as db:
        ...
        if idempotency_key is not None:
            existing = db.scalar(
                with_lock(select(Task).where(Task.sender_id == sender_id, Task.idempotency_key == idempotency_key))
            )
...
def _find_attempt_for_token(db: Session, task_id: str, token: str) -> Attempt | None:
    return db.scalar(
        with_lock(select(Attempt).where(Attempt.task_id == task_id, Attempt.claim_token_hash == secret_hash(token)))
    )


def heartbeat(task_id: str, agent_id: str, claim_token: str) -> str:
    with immediate_transaction() as db:
        task = db.scalar(with_lock(select(Task).where(Task.id == task_id)))
